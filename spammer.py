...
def main():
    ...
    if number == "1":
        ...
        print(f"\n{BRIGHT}{RED}[*] Error number!{RESET_ALL}")
        sleep(1)
        main()
...
def update():
    ...
    if update.lower() == "y":
        ...
        try:
            file.write(get("https://raw.githubusercontent.com/Flazers/sm_bot/master/spammer.py").content)
            file.close()
            system("spammer")
        except UnboundLocalError:
            system("cd $HOME && rm -rf spammer && git clone https://github.com/Flazers/sm_bot/ && cd spammer && sh install.sh")
    else:
        main()
